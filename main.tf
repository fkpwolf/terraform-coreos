...
# nodes
resource "libvirt_volume" "coreos_production_qemu" {
  name   = "coreos_production_qemu"
  source = "/mnt/kvm-images/fedora-coreos-34.20210725.3.0-qemu.x86_64.qcow2"
  pool = "kvm-images"
}
...
resource "libvirt_volume" "coreos-disk" {
  ...
  count            = var.hosts
  base_volume_id = libvirt_volume.coreos_production_qemu.id
  pool             = "default"
  format           = "qcow2"
}
